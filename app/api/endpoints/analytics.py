...
from pydantic import BaseModel, ConfigDict #NEW
...
class AnalyticsResponse(BaseModel): #NEW
    user_id: int
    period: str
...
class PatternAnalyticsResponse(BaseModel): #NEW
    user_id: int
    period: str
    total_cravings: int = 0
    day_of_week_distribution: Optional[dict] = {}
    peak_days: Optional[list] = []
...
class TimeOfDayAnalyticsResponse(BaseModel):
    ...
    average_intensity_by_time: Dict
    peak_time: Optional[str] = None
    insights: List
    message: Optional[str] = None  # Optional message
    model_config = ConfigDict(from_attributes=True)
class IntesityAnalyticsResponse(BaseModel): #NEW
    user_id: int
    period: str
    average_intensity: Optional[float] = 0
    intensity_trend: str
...
@router.get("/user/{user_id}/summary", response_model=AnalyticsResponse) #NEW
...
@router.get("/user/{user_id}/patterns", response_model=PatternAnalyticsResponse) #NEW
async def get_craving_patterns(
    ...
    return PatternAnalyticsResponse (
        ...
        peak_days = peak_days,
        insights = generate_insights(cravings