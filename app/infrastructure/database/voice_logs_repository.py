...
class VoiceLogsRepository:
    ...
    def get_by_id(self, voice_log_id: int) -> Optional[VoiceLog]:
        record = self.db_session.query(models.VoiceLog).filter(
            models.VoiceLog.id == voice_log_id,
            models.VoiceLog.is_deleted == False
        ).first()
        if record:
    ...
    def list_by_user(self, user_id: int) -> List[VoiceLog]:
        records = self.db_session.query(models.VoiceLog).filter(
            models.VoiceLog.user_id == user_id,
            models.VoiceLog.is_deleted == False
        ).all()
        return [VoiceLog(**r.__dict__) for r in records]

    def update(self, updated_log: VoiceLog) -> Optional[VoiceLog]:
        record = self.db_session.query(models.VoiceLog).get(updated_log.id)
    ...
    def soft_delete(self, voice_log_id: int) -> bool:
        record = self.db_session.query(models.VoiceLog).get(voice_log_id)
        if not record:
            return False
